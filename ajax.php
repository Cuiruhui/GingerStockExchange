...
function Getw2FavStock()
{
    ...
    $html .= '<tr align="center" height="28"><th width="40" >选择</th><th>代码</th><th>名称</th><th >最新价</th><th >成交量(手)</th><th>涨跌</th><th>漲跌%</th><th >买价</th><th>卖价</th><th >开盘价</th><th>最高价</th><th >最低价</th><th >昨收价</th><th>时间</th><th width="70">操作</th></tr>';
		...
		if($rows = $xltm->SQL->GetRows("Select * from `user_stock` where user='{$user}' order by stock_code asc"))
		{
			...
			if($StockList)
			{
				...
				foreach($messages as $l)
				{
					...
					$html .= '<tr align="center" height="28" bgcolor="'.$bgcolor.'" onMouseOver="this.style.background=\'#FDF0D7\';" onMouseOut="this.style.background=\''.$bgcolor.'\';">';
					...
					{
						$html.=' onClick="location.href=\'wmain.php?do=order&stockcode='.$N[0].'\';"';
					}
					$html.=' /></td> ';
        			$html .= '</tr> ';
					$i++;
...
function meiguquote()
{
	...
	if($rows=$xltm->SQL->GetRow("select type from `usstock_code` where code='{$stockcode}'"))
	{
		$stockarea=$rows['type'];
...
function GetallStockList()
{
	...
	$query = "select code,name,can_bull,can_bull_up,can_bull_down  from `stock_code` ";
	if($rows=$xltm->SQL->GetRows($query))
	{
		...
		foreach($rows as $row)
		{
			//0代码 1名称 3关盘 4停牌 5买入  8买升 9买跌
			$r .= ($r==''?'':'$').$row['code'].'|'.$row['name'].'|'.$row['can_bull'].'|'.$row['can_bull_up'].'|'.$row['can_bull_down'];
		}
		$res .= '^' . $r;
	}
	else
...
{	
	...
	$canmoney = bcmul("$usercancash", $xltm->config['cost_exchange_rate'], 2);
	$cash=0;
	$zymoney=0;
	if($rows=$xltm->SQL->GetRow("select cash,frozencash from `user_users` where username='{$username}'"))
	{
		$cash=$rows['cash'];
		
	}
	if($lc=$xltm->SQL->GetRows("select *  from `buy_deal` where sell='0' and user='{$xltm->user_info['username']}' order by bull_trust_time desc"))
	{
		foreach($lc as $k)
		{
			$zymoney += $k['bull_money'];
		}
		$zymoney=$zymoney/100;
	}
	
	$allmoney=$cash.'^'.$zymoney.'^'.$canmoney.'^'.'0'.'^'.$nowtime;
...
function GetUserFavStock()
{
	...
	$StockList = '';
	$FavIDList = '';
	$html = '<table width="99%"  align="center" cellpadding="0" cellspacing="0"  class="table table-hover">';
    $html .= '<tr align="center"><th width="40" class="blue">选择</th><th class="blue">代码</th><th class="blue">名称</th><th class="blue">最新价</th><th class="blue">成交量(手)</th><th class="blue">涨跌</th><th class="blue">漲跌%</th><th class="blue">买价</th><th class="blue">卖价</th><th class="blue">开盘价</th><th class="blue">最高价</th><th class="blue">最低价</th><th class="blue">昨收价</th><th class="blue">时间</th><th width="70" class="blue">操作</th></tr>';
	$user = $xltm->user_info['username'];
	if($user)
	{
		if($rows = $xltm->SQL->GetRows("Select * from `user_stock` where user='{$user}' order by stock_code asc"))
...
function GetStockList()
{
	...
	if($rows=$xltm->SQL->GetRows($query))
	{
		...
		foreach($rows as $row)
		{
			...
			$r .= ($r==''?'':'$').$row['code'].'|'.$row['name'].'|'.$row['pinyin'].'|'.$row['inf'].'|'.$row['stop_pai'].'|'.$row['can_bull'].'|'.$row['can_sell'].'|'.$row['dc'].'|'.$row['can_bull_up'].'|'.$row['can_bull_down'].'|'.$row['bk_list'].'|'.$row['type'];
		}
		$res .= '^' . $r;
	}
	else
...
function GetwapFavStock()
{
    ...
    $html .= '<tr align="center" bgcolor="#C5E2FB"><th width="40">选择</th><th>代码</th><th>名称</th><th>新价</th><th>漲跌%</th><th width="70">操作</th></tr>';
	$user = $xltm->user_info['username'];
	if($user)
	{
		if($rows = $xltm->SQL->GetRows("Select * from `user_stock` where user='{$user}' order by stock_code asc"))
		{
			...
			if($StockList)
			{
				...
				foreach($messages as $l)
				{
					...
					{
						$html.=' onClick="location.href=\'waporder.php?code='.$N[0].'\';"';
					}
					$html.=' /></td> ';
        			$html .= '</tr> ';
					$i++;
	...
	if($rows=$xltm->SQL->GetRows($query))
	{
		...
		foreach($rows as $row)
		{
			...
			foreach($messages as $l)
			{
				...
				//0代码 1名称 2拼音 3关盘 4停牌 5买入 6卖出 7点差 8买升 9买跌 10板块 11sz or sh 12现价 13买1 14卖1 15涨跌幅 16今开 17最高 18最低 
				//19昨收 20涨跌额 
				$r .= ($r==''?'':'$').$row['code'].'|'.$row['name'].'|'.$row['pinyin'].'|'.$row['inf'].'|'.$row['stop_pai'].'|'.$row['can_bull'].'|'.$row['can_sell'].'|'.$row['dc'].'|'.$row['can_bull_up'].'|'.$row['can_bull_down'].'|'.$row['bk_list'].'|'.$row['type'].'|'.$xianjia.'|'.$N[12].'|'.$N[22].'|'.$zdf.'|'.$N[2].'|'.$N[5].'|'.$N[6].'|'.$N[3].'|'.$zde;
			}
		}
		$res .= '^' . $r;
	}
...
function GetusStockList()
{
	...
	//统计总记录数
	$query = "select count(id) as totalrecord from `usstock_code` where 1=1 {$where} order by code asc";
	//exit($query);
	if($row=$xltm->SQL->GetRow($query))
	{
		if(is_numeric($row['totalrecord']) && $row['totalrecord']>0)
	...
	//分页设置
	$res = $total.'^';
	$query = "select * from `usstock_code` where 1=1 {$where} order by code asc limit ".($page-1)*$pagesize.",".$pagesize;
	if($rows=$xltm->SQL->GetRows($query))
	{
		$total1 = count($rows); //本次共有记录数
		$res.=$total1;
		...
		{
			//0代码 1名称 2拼音 3关盘 4停牌 5买入 6卖出 7点差 8买升 9买跌 10板块
			$r .= ($r==''?'':'$').$row['code'].'|'.$row['name'].'|'.$row['type'].'|'.$row['can_bull'].'|'.$row['can_sell'].'|'.$row['dc'].'|'.$row['can_bull_up'].'|'.$row['can_bull_down'];
		}
		$res .= '^' . $r;
	}
	else

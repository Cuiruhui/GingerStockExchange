...
switch($types)
{
	...
	case 'add':
		...
		$sql = "INSERT INTO `payment` VALUES ('0', '{$orderno}', '{$xltm->user_info['username']}', 'Buy', '{$agent}', '{$merid}', '0', null, '{$money}', 'CNY', null, null, '{$add_time}', null, null, '{$payid}')";
	...
	{
		if($pay_config = $xltm->SQL->GetRow("Select id,merid,connecturl,callbackurl,secretkey,name from `pay_config` where type='IN' and accounttype='1' and (payinterfacename='易宝' or payinterfacename='购宝支付') and active='1' and agent='{$agent}'"))
		{
			$mp = $pay_config['id'];
			$merid = $pay_config['merid']; //商户代码
			$webid = $merid;
		...
		{
			echo '<script>alert("在线支付升级中,冲值请联系在线客服,如有不便,敬请谅解!"); history.back(-1);</script>';
			exit();
		}
	}

...
switch($types)
{
	...
	default:
		...
		if(!$xltm->AtmTime())
		{
			mb("对不起，提款时间段为开盘日的 上午{$xltm->config['atm_AM_Start']}-{$xltm->config['atm_AM_End']} 下午{$xltm->config['atm_PM_Start']}-{$xltm->config['atm_PM_End']} 敬请谅解！","",0);
			exit();		
		}
		...
		if($types=='AddAtm')
		{
			...
			if($today_trade_money == 0 || $liuchange != 0){
				$cancash = $cancash * $xltm->config['atm_p'] / 100;
				}
		...
		if($xltm->user_info['demo']=='0') //正式帐号才计算
		{
			...
			if($today_trade_money == 0 || $liuchange != 0){
				$usercancash = $usercancash * $xltm->config['atm_p'] / 100;
				}
		...
		$time1 = $xltm->config['atm_AM_Start'];
		$time2 = $xltm->config['atm_AM_End'];
		$time3 = $xltm->config['atm_PM_Start'];
		$time4 = $xltm->config['atm_PM_End'];

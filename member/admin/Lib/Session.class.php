...
class Session {
	...
	function clear_old_sessions() {
		...
		$this->xltm->SQL->Execute("DELETE from `user_sessions` WHERE time<'{$time_minus_defined}'");
	}
	...
	function create_session($username, $password, $user_code, $remember = false) {
		$time = time();
		// 获取用户ID
		$user_id=$this->xltm->SQL->GetRow("select id from user_admin where username='{$username}'");
		// 生成session的ID
		...
		$this->xltm->SQL->Execute("INSERT INTO `user_sessions` set id='{$unique_id}',value='{$value}',time='{$time}',type='5'");
		// 更新用户表的最后登陆 last_session 与 last_login时间
		$this->xltm->SQL->Execute("UPDATE `user_admin` SET `last_session`='{$value}',last_login='".time()."' WHERE `username`='{$username}'");
		//永久写入
	...
	function fetch_session($information) {
		...
		$user_session=$this->xltm->SQL->GetRow("select last_session from user_admin where id='{$information[0]}'");
		//根据用户的last_session值得到sessions表的信息
		$session_info=$this->xltm->SQL->GetRow("SELECT * FROM user_sessions WHERE id='{$information[2]}' and value='{$user_session['last_session']}' and type='5'");
		//print_r($session_info);
		//print_r(sha1($session_info['time']));
		//如果用户的session有效
		if ($session_info['id'] != '') {
		...
		if (is_array($information)) {
			...
			&& preg_match('/^[a-fA-F0-9]{40}$/', $information[2])) {
				   // 验证session
					...
					// 获取用户users数据表的信息
					$row=$this->xltm->SQL->GetRow("select * from user_admin where id='{$information[0]}'");
					...
					// 更新这session时间
					$upTime=time();
					$this->xltm->SQL->Execute("update user_sessions set time='$upTime' where id='{$information[2]}' and type='5'");
					return true;
				}
				else {
					return false;

	...
	default:
		if($type=='AddFrom')
		{
			...
			if ($bill_type == '滑价修正'){		//滑价修改，只处理持仓股，也就是未卖出的。而且，入费扣费都一样的处理。
				...
				$user_info = $xltm->SQL->GetRow("select * from `user_users` where agent='{$_POST['agent']}' and username='{$_POST['username']}'");
...
//print_r($dc_money);exit;
			...
			$xltm->SQL->Execute("update `user_users` set money=money+{$money},cash=cash+{$money} where agent='{$_POST['agent']}' and username='{$_POST['username']}'");

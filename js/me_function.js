		...
		$.ajax({
			...
			success: function(response){
				...
				for(var i=1; i<bulls.length; i++){
					...
					var bull_cost_money=inf[3]; //买入手续费
					var sell_cost=inf[4]; //卖出手续费率
					var save_money=inf[5]; //留仓费
					var dc_money = inf[6]; //点差费金额
...
function sale_buy(code,name,num,id,limit_time,limit_cost)
{
	...
	{
	dourl = 'ussell.php?stockcode=' + code + '&stockname=' + name + '&num=' + num + '&id=' + id;
	
	}
	
	//判断股票是否可卖
	$.ajax({
		url:'usbuy.php?type=buytime&id='+id,
		type:'GET',
		async:false,
		timeout:5000,
		success:function(data){
	...
	if(!cansell)
	{
		...
		{
			ymPrompt.win({message:dourl,width:400,height:270,title:'平仓股票' + name + '[' + code + ']',iframe:true,handler:function(){self.location.href=self.location.href+'?rnd='+Math.random();}})
		}
	}
	else
	{
	...
	$.ajax({
	   ...
	   success:function(rescheck){
			if (rescheck.indexOf('true')==-1) {
				ymPrompt.errorInfo({title:'自选股',message:rescheck});
				return false;
			}
			else {
				//添加自选股
				$.ajax({
					   ...
					   cache: false,
					   success:function(res){
							ymPrompt.alert({title:'自选股',message:res});
					   }
				});
			}
	   }
...
function Kxian(id)
	{
		ymPrompt.win({message:'http://finance.sina.com.cn/flash/cn.swf?symbol='+id,width:500,height:450,title:'K线图',iframe:true});
	}

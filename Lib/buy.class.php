...
class buy{
	...
	function bull_stock()
	{
		...
		if($row=$this->xltm->SQL->GetRow("SELECT * FROM `stock_code` WHERE code='$code'")){
			...
			$nowcje=$Net[9]/10000;
			$zuidijiaoyi=$this->xltm->config['turnover_min'];
			if($nowcje<=$zuidijiaoyi)
			{
				exit("false|成交额低于{$zuidijiaoyi}万,暂停交易！");
			}
			...
			//成交额大于五千万 的点差率。
			if ( $Net[9] > 2000000){		
				$dc_num1 = $this->xltm->config['dc_wan200'];
			}
			if ( $Net[9] > 5000000){		
				$dc_num1 = $this->xltm->config['dc_wan500'];
			}
			if ( $Net[9] > 8000000){		
				$dc_num1 = $this->xltm->config['dc_wan800'];
			}
			if ( $Net[9] > 12000000){		
				$dc_num1 = $this->xltm->config['dc_wan1200'];
			}
			if ( $Net[9] > 20000000){		
				$dc_num1 = $this->xltm->config['dc_wan2000'];
			}
			if ( $Net[9] > 50000000){		
	...
	*/
	function auto_bull_stock($trustID)
	{
		if($row=$this->xltm->SQL->GetRow("SELECT * FROM `buy_trust` WHERE id='{$trustID}' and app='1'"))
		{
			...
			$cur_price = $Net[3];
			//买升并且委托价大于等于当前价或者买跌委托价等于当前价买入
			if(($buy_type==1 && $new_price>=$cur_price) || ($buy_type==2 && $new_price<=$cur_price))
			{
			}
			else //委托
			{
...
if ( $dc_money == 0	){
	...
	//自动平仓股票
	function auto_sale_stock($trustID)
	{
		...
		if($row=$this->xltm->SQL->GetRow("select * from `buy_trust` where id='{$trustID}' and trust_type='2' and app='1'"))
		{
			...
			preg_match("/\"(.+?)\";/is",$html_data,$data);
			$Net=split(',',$data[1]);
			if($Net[8]<=0 || $Net[7]<=0) return '';
			$cur_price2 = $Net[3];
			
			//买升并且委托价大于等于当前价或者买跌委托价等于当前价买入
			if(($row['buy_type']==1 && $row['price']<=$cur_price2) || ($row['buy_type']==2 && $row['price']>=$cur_price2))
			{
			}
			else //委托
			{

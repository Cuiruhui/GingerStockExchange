...
switch($types)
{
	...
	case 'log':
		...
		$xltm->tpls->LoadTemplate("./tmpfiles/w2atmlog.html",false);
	...
	default:
		...
		if(!$xltm->AtmTime())
		{
			mb("对不起，提款时间段为开盘日的 中午{$xltm->config['atm_AM_Start']}-{$xltm->config['atm_AM_End']} 下午{$xltm->config['atm_PM_Start']}-{$xltm->config['atm_PM_End']} 敬请谅解！","",0);
			exit();		
		}
		if($types=='AddAtm')
		{
			...
			mb("提款信息保存成功.请耐心等待管理员打款!","w2atm.php?type=log",1);
			exit();
		}
		...
		$xltm->tpls->LoadTemplate("./tmpfiles/w2atm.html");
		$xltm->tpls->Show();
		break;
}

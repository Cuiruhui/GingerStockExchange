...
function calcDealSave()
{
	...
	if($rows=$xltm->SQL->GetRows("select * from `buy_deal` where sell='0' order by id limit ".($page-1)*$pagesize.",".$pagesize))
	{
		...
		foreach($rows as $r)
		{
			...
			//留仓量
			$save_money = $r['bull_price'] * $r['bull_num']*100;
			//留仓手续率
			...
			else //添加新记录
			{
				...
				//更新留仓的费用和代理商留仓费的佣金
				$xltm->SQL->Execute("Update `buy_deal` set save_day='{$save_day}',save_money='{$cost_save_money}',agent_save_money='{$agent_cost_save_money}'  where id='{$r['id']}'");
				// 扣除用户的保证金
